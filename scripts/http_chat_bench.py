#!/usr/bin/env python3
"""Small dependency-free OpenAI-chat benchmark for DiffusionGemma/vLLM.

vLLM's generic completions path can be misleading for DiffusionGemma; this
script benchmarks /v1/chat/completions with the fixed chat template that the
...
PROMPTS = [
    "Write a compact technical summary of FP8 KV cache benefits for serving NVFP4 MoE models. Use bullet points.",
    "Explain why native CUTLASS or FlashInfer kernels matter for SM121 FP4 inference. Keep it concise.",
    "List practical checks for validating a vLLM container before publishing it. Include evidence files.",
    "Give a short deployment note for running DiffusionGemma on a GB10-class ARM64 system.",
]
...
def main():
    ...
    ap.add_argument("--requests-per-concurrency", type=int, default=8)
    ap.add_argument("--max-tokens", type=int, default=256)
    ap.add_argument("--timeout", type=int, default=600)
    args = ap.parse_args()
    ladder = args.concurrency or [1,2,4,5,8,16]
